...
- Add opt-in rigid-soft DAT augmented-Lagrangian plane constraints to `SolverVBD` and per-geometry penetration reporting to `newton.exp`.
- Add opt-in rigid-style normal/tangential augmented-Lagrangian constraints for direct rigid-soft contact pairs to `SolverVBD` and the `newton.exp --contact-alm` flag.
- Add opt-in checkpointed rigid-soft DAT to `SolverVBD` and `newton.exp --dat-checkpointed`, preserving Contact-ALM or DAT-ALM proposals between safe collision-refresh checkpoints, plus `CollisionPipeline.collide_soft()` for refreshing rigid-soft rows without changing rigid-rigid contacts.
- Add explicit opt-in controls for the experimental VBD DAT pinch exemption and finite-gap bounded-advance relaxations; strict truncation is now the default.
