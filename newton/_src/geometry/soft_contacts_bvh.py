...
.. note:: **Dense-query contract / consumer responsibility.** Because emission is unfiltered, a
    ...
    this local orientation test before applying forces, while retaining every row in Contacts.
    This query does not provide continuous collision detection or complete-primitive DAT.
    Recovering an already-intersecting surface is outside this local validity rule; it
...
def launch_soft_bvh_contacts(
    *,
    ...
    parallel epsilon); pass ``None`` when the model has neither triangles nor edges. The TV/EE
    passes are then skipped and VT alone covers the soft features."""
    ...
    # attempted count so diagnostics distinguish an unprovisioned buffer from an empty query.
    if candidate_max == 0:
        return
