...
def _build_soft_particle_rigid_contact_pairs(model: Model, shape_mask: np.ndarray | None = None) -> wp.array[wp.vec2i]:
    ...
    the BVH full-surface query replaces the legacy closest-point records for those shapes,
    so they must not also appear here.
    """
    ...
    return _world_compatible_pairs(
        model.particle_world.numpy(), model.shape_world.numpy(), world_count, model.device, shape_ok=shape_mask
    )
...
def _count_soft_particle_rigid_contact_pairs(model: Model) -> int:
    ...
    from the legacy pairs (their BVH records replace the legacy ones).
...
def _raise_on_unprovisioned_full_surface_meshes(model: Model, capable: np.ndarray) -> None:
    ...
    raise ValueError(
        ...
        f"signed-distance field: {missing}. The full-surface SDF backend samples each rigid "
        f"mesh/convex shape's SDF for particle, edge, and face contacts. "
        f"Provision an SDF before ModelBuilder.finalize(), any one of these ways:\n"
...
class CollisionPipeline:
    ...
    def __init__(
        ...
        *,
        ...
        rigid_soft_full_surface_mesh_backend: Literal["sdf", "bvh"] = "sdf",
        ...
        Args:
            ...
            enable_rigid_soft_full_surface_contact: Generate soft contacts over the full soft-mesh
                ...
                the back-end selected independently by ``rigid_soft_full_surface_mesh_backend``. Consumed only by
            ...
            rigid_soft_full_surface_mesh_backend: Contact-query back-end for rigid **mesh/convex** shapes.
                Used only when ``enable_rigid_soft_full_surface_contact`` is ``True``.
                ``"sdf"`` (default) samples the provisioned SDF for particle contacts and minimizes
                it over soft edges and faces; participating meshes need an SDF.
                ``"bvh"`` runs dense vertex/edge/face queries against the mesh geometry, with
                soft BVHs refitted automatically by :meth:`collide`. Without full-surface contact,
                meshes always use the legacy nearest-triangle particle query and need no texture SDF.
            full_surface_bvh_contact_headroom: Records reserved in the default ``soft_contact_max``
                ...
                Increase this value or set ``soft_contact_max`` explicitly when queries overflow.
                Defaults to 4. Dense queries do not provide continuous collision detection.
        ...
        if rigid_soft_full_surface_mesh_backend not in ("bvh", "sdf"):
            raise ValueError(
                f"rigid_soft_full_surface_mesh_backend must be 'bvh' or 'sdf', got {rigid_soft_full_surface_mesh_backend!r}"
            )
        ...
        self.rigid_soft_full_surface_mesh_backend = rigid_soft_full_surface_mesh_backend
        self.full_surface_bvh_contact_headroom = full_surface_bvh_contact_headroom

        _full_surface_capable_mask = (
            _full_surface_capable_shape_mask(model)
            if enable_rigid_soft_full_surface_contact and model.shape_count > 0
            else None
        )
        if rigid_soft_full_surface_mesh_backend == "sdf" and _full_surface_capable_mask is not None:
            _raise_on_unprovisioned_full_surface_meshes(model, _full_surface_capable_mask)

        # Shapes handled by the BVH full-surface back-end. None when the feature is off, the SDF
        # back-end is selected, or no mesh/convex shape participates. Other shapes retain their
        # particle and SDF edge/face passes.
        _full_surface_bvh_shape_mask: np.ndarray | None = None
        if (
            enable_rigid_soft_full_surface_contact
            and rigid_soft_full_surface_mesh_backend == "bvh"
        ...
        self.soft_rigid_contact_pairs = _build_soft_particle_rigid_contact_pairs(
            model,
            shape_mask=(~_full_surface_bvh_shape_mask if _full_surface_bvh_shape_mask is not None else None),
        )
        ...
        if enable_rigid_soft_full_surface_contact:
            ...
            self.soft_edge_rigid_pairs = _build_soft_edge_rigid_contact_pairs(model, _full_surface_sdf_shape_mask)
            self.soft_face_rigid_pairs = _build_soft_face_rigid_contact_pairs(model, _full_surface_sdf_shape_mask)
        else:
        ...
        if _full_surface_bvh_shape_mask is not None:
            if model.tri_count > 0 or model.edge_count > 0:
                (
                ...
                # No soft faces or edges: the VT pass alone covers the soft features.
                self._full_surface_bvh_rigid_vertex_table = wp.array(
                    np.empty((0, 2), np.int32), dtype=wp.vec2i, device=device
                )
                self._full_surface_bvh_rigid_vertex_normals = wp.array(
            ...
            self._full_surface_bvh_vt_pairs = _world_compatible_pairs(
                ...
                device,
                shape_ok=_full_surface_bvh_shape_mask,
            )
        ...
        # Construct before graph capture; self-contact can subsequently configure the shared trees.
        self._soft_self_contact_detector: TriMeshCollisionDetector | None = (
            TriMeshCollisionDetector(model) if self._full_surface_bvh_needs_detector else None
        )
    ...
    def contacts(self) -> Contacts:
        ...
        if self.requires_grad and self._full_surface_bvh_thread_count > 0:
            contacts._soft_contact_bvh_candidate_count = wp.zeros_like(self._full_surface_bvh_candidate_count)
            contacts._soft_contact_bvh_candidates = wp.zeros_like(self._full_surface_bvh_candidates)
    ...
    def _ensure_soft_self_contact_detector(self) -> TriMeshCollisionDetector:
        ...
        if not self._soft_self_contact_enabled:
    ...
    def refit_soft_self_contact_bvh(self, new_pos: wp.array[wp.vec3], *, rebuild: bool = False) -> None:
        """Refit (or fully rebuild) the shared soft-contact BVHs to ``new_pos``.

        :meth:`collide` automatically refits before rigid-soft BVH and self-contact detection.
        ...
        detector = self._soft_self_contact_detector
        if detector is None:
            raise ValueError("configure soft self-contact or enable the full-surface BVH backend first.")
        if rebuild:
    ...
    def collide(
        ...
        *,
        ...
        bvh_candidate_count = self._full_surface_bvh_candidate_count
        bvh_candidates = self._full_surface_bvh_candidates
        if self.requires_grad and self._full_surface_bvh_thread_count > 0:
            if contacts._soft_contact_bvh_candidates is None or len(contacts._soft_contact_bvh_candidates) < len(
                bvh_candidates
            ):
                raise ValueError("Allocate differentiable BVH contacts with this CollisionPipeline.contacts().")
            bvh_candidate_count = contacts._soft_contact_bvh_candidate_count
            bvh_candidates = contacts._soft_contact_bvh_candidates
        ...
        if state.particle_q and self.soft_contact_max > 0 and self.soft_contact_pair_count > 0:
            # keep legacy behavior: process closest-point particle-to-mesh query
            # with sdf only when full surface is on and backend selection is "sdf"
            mesh_contacts_use_sdf = (
                self.enable_rigid_soft_full_surface_contact and self.rigid_soft_full_surface_mesh_backend == "sdf"
            )
            wp.launch(
        ...
        if self.enable_rigid_soft_full_surface_contact and state.particle_q:
            # refit the shared soft BVHs unless self-contact already did it
            if self._full_surface_bvh_needs_detector and not soft_self_contact:
                self._soft_self_contact_detector.refit(state.particle_q)
            launch_soft_bvh_contacts(
                ...
                candidate_count=bvh_candidate_count,
        ...
        if self._verify_buffers and state.particle_q:
            wp.launch(
