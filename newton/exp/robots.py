...
def add_franka(builder, *, enable_finger_mimic: bool = False, collapse_fixed_joints: bool = False, base_z: float = 0.0):
    ...
    Args:
        ...
        enable_finger_mimic: Keep the URDF/USD ``<mimic>`` coupling of finger joint 2 to
            joint 1. Off by default: Newton main (#4014) now hands that coupling to
            ``SolverVBD.project_joint_mimics``, which is unstable with the legacy
            penalty joints used here, and IsaacLab's Franka drives both fingers
            independently to the same target anyway (``panda_finger_joint.*``).
    ...
    if not enable_finger_mimic:
        for j in range(joint_start, builder.joint_count):
            if builder.joint_mimic_joint[j] >= 0:
                builder.set_joint_mimic(j, None)
    return (
